from hybrid_loss_stage1 import HybridMatchingLoss
import random
import pickle
...
class A_llmrec_model(nn.Module):
    def __init__(self, args):
        ...
        self.mse = nn.MSELoss()
        self.hybrid_loss = HybridMatchingLoss(tau=0.07, alpha_mse=0.5, alpha_clip=0.5)
        ...
        if args.pretrain_stage2 or args.inference:
            ...
            self.log_emb_proj = nn.Sequential(
                nn.Linear(self.rec_sys_dim, self.llm.llm_model.config.word_embed_proj_dim),
                nn.LayerNorm(self.llm.llm_model.config.word_embed_proj_dim),
                nn.LeakyReLU(),
                nn.Linear(self.llm.llm_model.config.word_embed_proj_dim, self.llm.llm_model.config.word_embed_proj_dim)
            )
            ...
            self.item_emb_proj = nn.Sequential(
                nn.Linear(128, self.llm.llm_model.config.word_embed_proj_dim),
                nn.LayerNorm(self.llm.llm_model.config.word_embed_proj_dim),
                nn.GELU(),
                nn.Linear(self.llm.llm_model.config.word_embed_proj_dim, self.llm.llm_model.config.word_embed_proj_dim)
            )
    ...
    def load_model(self, args, phase1_epoch=None, phase2_epoch=None):
        # если ничего не передали, берём те эпохи, для которых есть файлы
        if phase1_epoch is None:
            phase1_epoch = 10      # у тебя файлы All_Beauty_sasrec_10_...
        if phase2_epoch is None:
            phase2_epoch = 3       # у тебя есть ..._opt_3_log_proj.pt
    ...
    def pre_train_phase1(self,data,optimizer, batch_iter):
        ...
        while start_inx < len(log_emb_):
            ...
            matching_loss = self.hybrid_loss(pos_text_matching, pos_text_matching_text) + self.hybrid_loss(neg_text_matching, neg_text_matching_text)
            ...
            total_loss = (loss + matching_loss + 0.5*reconstruction_loss + 0.2*text_reconstruction_loss) / accumulation_steps
            total_loss.backward()
            if iterss % accumulation_steps == 0:
                optimizer.step()
                optimizer.zero_grad()
    ...
    def generate(self, data):
        ...
        with torch.no_grad():
            ...
            with torch.cuda.amp.autocast():
                ...
                # Приводим скрытую размерность inputs_embeds к размеру log_emb
                hidden_size = log_emb.size(-1)
                if inputs_embeds.size(-1) != hidden_size:
                    if inputs_embeds.size(-1) > hidden_size:
